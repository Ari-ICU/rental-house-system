/**
 * ─────────────────────────────────────────────
 *  logger.ts  –  Zero-dependency colored logger
...
const colorize = (color: string, text: string | number): string => `${color}${text}${RESET}`;
const bold = (text: string): string => `${BOLD}${text}${RESET}`;
const dim = (text: string): string => `${DIM}${text}${RESET}`;
const timestamp = (): string => colorize(FG.gray, `[${new Date().toISOString()}]`);
...
const log = (level: string, message: string, ...args: any[]): void => {
...
const logger = {
    ...
    success: (msg: string, ...a: any[]) => log('success', msg, ...a),
    ...
    http: (msg: string, ...a: any[]) => log('http', msg, ...a),

    /** Database operations */
    db: (msg: string, ...a: any[]) => log('db', msg, ...a),

    /** Server lifecycle events */
    server: (msg: string, ...a: any[]) => log('server', msg, ...a),
