...
const getHash = (str: string, key: string): string => {
        ...
        if (settings && settings.paymentBakongAccountId) {
            try {
                const reqData = new IndividualInfo(
                    ...
                    amountToPay,
                    'USD',
                    `Room ${bill.rental?.roomNumber || 'Unknown'}`,
                    'Phnom Penh'
                );
...
export const simulatePaymentSuccess = async (req: Request, res: Response): Promise<any> => {
    try {
