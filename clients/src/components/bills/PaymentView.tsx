...
export default function PaymentView({ billId, amount, onClose, onPaymentSuccess }: PaymentViewProps) {
    ...
    return (
        <div className="p-6 sm:p-10 flex flex-col items-center space-y-6 animate-in zoom-in-95 duration-300">
            <div className="w-full flex justify-between items-center text-gray-500 dark:text-gray-400 mb-2">
                <button onClick={onClose} className="hover:text-indigo-600 dark:hover:text-indigo-400 transition flex items-center gap-2 font-bold px-4 py-2 bg-gray-50 dark:bg-slate-800 rounded-xl hover:bg-gray-100 dark:hover:bg-slate-700">
            ...
            {loading ? (
                <div className="h-64 flex flex-col items-center justify-center space-y-4">
                    <FaSpinner className="animate-spin text-4xl text-indigo-500 dark:text-indigo-400" />
                ...
                <div className="h-64 flex flex-col items-center justify-center space-y-4 text-center">
                    <div className="w-16 h-16 rounded-full bg-rose-50 dark:bg-rose-900/20 flex items-center justify-center text-rose-500 dark:text-rose-400 mb-2">
                        <FaExclamationTriangle className="text-3xl" />
                    </div>
                    <p className="font-black text-gray-800 dark:text-gray-200 text-lg">{lang === 'en' ? 'Gateway Error' : 'កំហុសច្រកទ្វារបង់ប្រាក់'}</p>
                ...
                <>
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-xl border-2 border-indigo-50 dark:border-indigo-900/30 hover:border-indigo-100 dark:hover:border-indigo-800/50 transition-colors">
                        {qrString && <QRCode value={qrString} size={240} fgColor="#312e81" />}
                    </div>

                    <div className="text-center space-y-1 bg-gray-50 dark:bg-slate-800/50 w-full rounded-2xl py-4 border border-dashed border-gray-200 dark:border-slate-700">
                        <p className="text-[10px] uppercase font-black text-gray-400 dark:text-gray-500 tracking-widest">
                    ...
                    <div className="pt-6 w-full flex flex-col items-center">
                        <p className="text-[10px] text-gray-400 dark:text-gray-500 mb-3 text-center px-4">
                        ...
                        <button
                            ...
                            disabled={simulating}
                            className="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-600 hover:text-white dark:hover:bg-emerald-600 dark:hover:text-white px-8 py-4 rounded-xl font-black text-sm flex items-center justify-center gap-3 transition-all active:scale-95 duration-300 w-full sm:w-auto shadow-sm"
                        >
